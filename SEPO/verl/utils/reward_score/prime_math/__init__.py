...
TUPLE_CHARS = "()[]"
VERBOSE_REWARD_TIMEOUTS = os.environ.get("VERL_REWARD_VERBOSE_TIMEOUTS", "0").lower() in {"1", "true", "yes", "on"}
MAX_SYMPY_EXPR_CHARS = int(os.environ.get("VERL_REWARD_MAX_SYMPY_EXPR_CHARS", "160"))
...
def should_allow_eval(expr: str):
    if len(expr) > MAX_SYMPY_EXPR_CHARS:
        return False
...
def _fast_grade_without_sympy(ground_truth_elem: str, given_elem: str) -> bool | None:
    """Return a cheap decision when Sympy is clearly unnecessary.

    This avoids spawning thousands of timeout-protected Sympy processes for
    simple wrong answers or prompt leakage during early RL training.
    """
    if ground_truth_elem == given_elem:
        return True

    if _is_float(ground_truth_elem) and _is_float(given_elem):
        return math.isclose(float(ground_truth_elem), float(given_elem), rel_tol=1e-7, abs_tol=1e-7)

    if re.fullmatch(r"[a-zA-Z]", ground_truth_elem or "") and re.fullmatch(r"[a-zA-Z]", given_elem or ""):
        return False

    given_lower = (given_elem or "").lower()
    text_markers = (
        "you are",
        "step ",
        "solution",
    ...
    if any(marker in given_lower for marker in text_markers):
        return False

    expr = f"({ground_truth_elem})-({given_elem})"
    if not should_allow_eval(expr):
        return False
    ...
    if (
        ...
        for ground_truth_elem, given_elem in zip(ground_truth_elems, given_elems, strict=True):
            if _is_frac(ground_truth_elem) and _is_frac(given_elem):
                ...
                fast_result = _fast_grade_without_sympy(ground_truth_elem, given_elem)
                if fast_result is not None:
                    is_correct = fast_result
                else:
                    try:
