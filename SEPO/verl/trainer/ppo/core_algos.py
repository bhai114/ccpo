...
# ===== GSPO negative_approx_kl 数据保存工具 =====
_gspo_log_call_count = 0
_GSPO_LOG_DIR = "./save_gspo_kl_logs"
_GSPO_RAW_SAVE_INTERVAL = 10  # 每隔多少次调用保存一次原始数据
_GSPO_KL_LOG_ENABLED = os.environ.get("VERL_SAVE_GSPO_KL_LOGS", "0").lower() in {"1", "true", "yes", "on"}


def _save_negative_approx_kl_data(
    negative_approx_kl: torch.Tensor,
    response_mask: torch.Tensor,
    negative_approx_kl_seq: torch.Tensor,
    seq_lengths: torch.Tensor,
):
    """保存 negative_approx_kl * response_mask 的分布数据到 txt 文件。

    保存内容：
    1. stats.txt: 每次调用都追加一行汇总统计（均值、标准差、分位数等）
    2. raw_token_step_{N}.txt: 每隔 N 步保存一次所有有效 token 的原始值
    3. raw_seq_step_{N}.txt: 每隔 N 步保存一次所有序列级平均值
    """
    if not _GSPO_KL_LOG_ENABLED:
        return

    import os

    global _gspo_log_call_count
    _gspo_log_call_count += 1
    step = _gspo_log_call_count

    # 只在第一个 rank 上保存（分布式训练时避免冲突）
    try:
    ...
    os.makedirs(_GSPO_LOG_DIR, exist_ok=True)

    # 提取有效 token 的值
    masked_kl = (negative_approx_kl * response_mask).detach()
    valid_values = masked_kl[response_mask.bool()].cpu().numpy()
    seq_values = negative_approx_kl_seq.detach().cpu().numpy()
    seq_len_values = seq_lengths.detach().cpu().numpy()

    # 1. 追加写入汇总统计
    stats_file = os.path.join(_GSPO_LOG_DIR, "stats.txt")
    write_header = not os.path.exists(stats_file)
    with open(stats_file, "a") as f:
        if write_header:
            f.write("step\tcount\tmean\tstd\tmin\tq25\tmedian\tq75\tmax\t"
                    "seq_mean\tseq_std\tseq_min\tseq_max\tavg_seq_len\n")
        if len(valid_values) > 0:
            q25, median, q75 = np.percentile(valid_values, [25, 50, 75])
            f.write(f"{step}\t{len(valid_values)}\t{valid_values.mean():.6f}\t{valid_values.std():.6f}\t"
                    f"{valid_values.min():.6f}\t{q25:.6f}\t{median:.6f}\t{q75:.6f}\t{valid_values.max():.6f}\t"
                    f"{seq_values.mean():.6f}\t{seq_values.std():.6f}\t{seq_values.min():.6f}\t{seq_values.max():.6f}\t"
                    f"{seq_len_values.mean():.1f}\n")

    # 2. 每隔 N 步保存原始数据（保留完整 (B, L) 结构）
    if step % _GSPO_RAW_SAVE_INTERVAL == 0:
        masked_kl_2d = masked_kl.cpu().numpy()       # (B, L)
        mask_2d = response_mask.detach().cpu().numpy()  # (B, L)

        # negative_approx_kl * response_mask 的完整矩阵，每行一个序列
        token_file = os.path.join(_GSPO_LOG_DIR, f"raw_token_step_{step}.txt")
        np.savetxt(token_file, masked_kl_2d, fmt="%.6f",
                   header=f"step={step}, shape={masked_kl_2d.shape}, "
                          f"each row is a sequence, padding positions are 0.0")

        # response_mask 矩阵，便于后续区分有效 token 和 padding
        mask_file = os.path.join(_GSPO_LOG_DIR, f"raw_mask_step_{step}.txt")
        np.savetxt(mask_file, mask_2d, fmt="%d",
                   header=f"step={step}, shape={mask_2d.shape}, 1=valid token, 0=padding")

        # 序列级平均值 (B,)
        seq_file = os.path.join(_GSPO_LOG_DIR, f"raw_seq_step_{step}.txt")
        np.savetxt(seq_file, seq_values, fmt="%.6f",
                   header=f"step={step}, shape=({len(seq_values)},), "
                          f"mean={seq_values.mean():.6f}, std={seq_values.std():.6f}")

        print(f"[GSPO KL Log] Step {step}: saved raw data (B={masked_kl_2d.shape[0]}, L={masked_kl_2d.shape[1]}) to {_GSPO_LOG_DIR}")
...
# 开关：True 使用 VD 稳健均值，False 使用简单均值。
# 为了做对比实验（普通均值 vs 稳健均值），支持通过环境变量覆盖：
#   VERL_USE_ROBUST_MEAN_VD=0      关闭 GSPO 重要性采样中的稳健均值
#   VERL_USE_ROBUST_MEAN_VD_ADV=0  关闭优势函数组内均值中的稳健均值
_USE_ROBUST_MEAN_VD = _parse_bool_env("VERL_USE_ROBUST_MEAN_VD", True)
_ROBUST_MEAN_MIN_TOKENS = 10  # 有效 token 少于此数时回退到简单均值

_USE_ROBUST_MEAN_VD_ADV = _parse_bool_env("VERL_USE_ROBUST_MEAN_VD_ADV", True)
_ROBUST_ADV_MIN_SAMPLES = 3      # 组内样本少于此数时回退到简单均值


def _get_robust_mean_vd_fn():
    """延迟导入 robustMeanEstimateVD，避免模块级副作用。"""
    import sys
    if "/gz-data/verl-gspo-VD-double" not in sys.path:
        sys.path.insert(0, "/gz-data/verl-gspo-VD-double")
    from mean_VD import robustMeanEstimateVD
    return robustMeanEstimateVD


def _robust_mean_scores(scores_np: np.ndarray) -> float:
    """对一组 response scores 使用 GNC-ADAPT-VD 计算稳健均值。

    Args:
        scores_np: (N,) 一组同 prompt 下多个 response 的 scores

    Returns:
        稳健均值估计
    """
    n = len(scores_np)
    if n < _ROBUST_ADV_MIN_SAMPLES:
        return float(np.mean(scores_np))

    robustMeanEstimateVD = _get_robust_mean_vd_fn()

    data = scores_np.reshape(1, -1).astype(np.float64)
    mu_init = np.mean(data, axis=1, keepdims=True)

    K = min(10, max(2, n // 5))
    opt_params = {
        'maxIter': 20,
        'tau': 3.0 * max(np.std(scores_np), 1e-6),
        'costThreshold': 1e-6,
        'K_quantiles': K,
    }
    params_vd = {'dim': 1}
    ...
    try:
        res = robustMeanEstimateVD(data, mu_init, opt_params, params_vd)
        return float(res['mu_est'].item())
    except Exception:
        return float(np.mean(scores_np))


def _robust_mean_vd_batch(
    negative_approx_kl: torch.Tensor,
    response_mask: torch.Tensor,
    seq_lengths: torch.Tensor,
    simple_mean: torch.Tensor,
) -> torch.Tensor:
    """对 batch 中每条序列用 GNC-ADAPT-VD 计算稳健均值。

    Args:
        negative_approx_kl: (B, L) token 级 log ratio
        response_mask: (B, L) 有效 token 掩码
        seq_lengths: (B,) 每条序列的有效长度
        simple_mean: (B,) 简单均值，作为 fallback
    ...
    B = negative_approx_kl.shape[0]
    device = negative_approx_kl.device
    dtype = negative_approx_kl.dtype

    kl_np = negative_approx_kl.detach().cpu().numpy()
    mask_np = response_mask.detach().cpu().numpy()
    simple_np = simple_mean.detach().cpu().numpy()

    result = np.zeros(B, dtype=np.float64)

    for i in range(B):
        valid = kl_np[i][mask_np[i] > 0.5]
        n = len(valid)

        if n < _ROBUST_MEAN_MIN_TOKENS:
            result[i] = simple_np[i]
            continue

        data = valid.reshape(1, -1)  # (1, N)
        mu_init = np.mean(data, axis=1, keepdims=True)  # (1, 1)

        K = min(10, max(2, n // 5))
        opt_params = {
            'maxIter': 20,
            'tau': 3.0 * max(np.std(valid), 1e-6),
            'costThreshold': 1e-6,
            'K_quantiles': K,
        }
        params_vd = {'dim': 1}
    ...
    with torch.no_grad():
        ...
        for idx in id2score:
            if len(id2score[idx]) == 1:
                ...
                if _USE_ROBUST_MEAN_VD_ADV:
                    scores_np = scores_tensor.detach().cpu().numpy()
                    id2mean[idx] = torch.tensor(_robust_mean_scores(scores_np), dtype=scores_tensor.dtype)
                else:
...
def compute_policy_loss_gspo(
    ...
    negative_approx_kl_seq_simple = torch.sum(negative_approx_kl * response_mask, dim=-1) / seq_lengths

    # 使用 GNC-ADAPT-VD 稳健均值替代简单均值
    if _USE_ROBUST_MEAN_VD:
        negative_approx_kl_seq = _robust_mean_vd_batch(
            negative_approx_kl, response_mask, seq_lengths, negative_approx_kl_seq_simple
        )
    else:
        negative_approx_kl_seq = negative_approx_kl_seq_simple

    # ===== 保存 negative_approx_kl 分布数据用于研究 =====
    _save_negative_approx_kl_data(negative_approx_kl, response_mask, negative_approx_kl_seq, seq_lengths)
    # ===== 保存结束 =====
