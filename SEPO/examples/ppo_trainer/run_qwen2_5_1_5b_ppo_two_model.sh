...
experiment_name=qwen2.5-1.5b-instruct-math500-twomodel-ppo-peerRwd-$backend
...
: "${PEER_REWARD_BASE:=0.2}"       # minimum reward for each role when verifier is correct
: "${PEER_WRONG_PENALTY_LAMBDA:=0.5}"
...
peer_reward_base=$PEER_REWARD_BASE
peer_wrong_penalty_lambda=$PEER_WRONG_PENALTY_LAMBDA
...
TWO_MODEL_CONFIG="
    ...
    +two_model.peer_reward_base_reward=$peer_reward_base \
    +two_model.peer_reward_wrong_penalty_lambda=$peer_wrong_penalty_lambda \
