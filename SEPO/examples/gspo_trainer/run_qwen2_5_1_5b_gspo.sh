...
# 默认 GPU/节点数量（如果环境变量未设置）
ARNOLD_WORKER_GPU=${ARNOLD_WORKER_GPU:-$(nvidia-smi -L 2>/dev/null | wc -l)}
ARNOLD_WORKER_NUM=${ARNOLD_WORKER_NUM:-1}

# wandb
backend=fsdp # fsdp, fsdp2, megatron
project_name=verl_grpo_math
experiment_name=qwen2.5-1.5b-inst-math500-gspo-$backend
default_local_dir=$DATA_ROOT/checkpoint/$project_name/$experiment_name
best_model_dir=$default_local_dir/best_model
...
# ===================================== Data/Model =====================================
train_files=/gz-data/verl-main/data/MATH/train.parquet
test_files=/gz-data/verl-main/data/MATH/test500.parquet

actor_model_path=/gz-data/models/Qwen2.5-1.5B-Instruct
...
# 保存策略：
# 1. 关闭固定频率 checkpoint 保存（trainer.save_freq=-1）
# 2. 每隔 trainer.test_freq 步跑一次验证
# 3. `verl/trainer/ppo/ray_trainer.py` 中的 BestModelSaver 会比较验证集准确率，
#    只有更高时才覆盖保存到 best_model 目录；否则不保存
printf 'Best validation model will be saved to: %s\n' "$best_model_dir"
