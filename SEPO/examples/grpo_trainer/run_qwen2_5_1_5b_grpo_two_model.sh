#!/usr/bin/env bash
# Two-model alternate training (Thinker + Solver) with GRPO and the peerRwd
# branch reward, based on two Qwen2.5-1.5B-Instruct models.
#
# This mirrors examples/gspo_trainer/run_qwen2_5_1_5b_gspo_two_model.sh in
# this repo, but switches the actor policy loss from GSPO to vanilla GRPO while
# keeping the peer self/peer rating reward plumbing.

set -x
export HF_ENDPOINT=https://hf-mirror.com
export WANDB_MODE=offline

# Keep the baseline plain and quiet.
...
RUN_ROOT=${RUN_ROOT:-/root/autodl-tmp/ccpo-verl-mulitAgent-peerRwd}
cd "$RUN_ROOT"
export PYTHONPATH=$CODE_ROOT:$CODE_ROOT/src/verl:$PYTHONPATH

# The peerRwd two-model recipe lives under gspo_trainer, but it is
# algorithm-agnostic; the GRPO/GSPO switch is controlled below.
...
# wandb
backend=fsdp
project_name=ccpo-grpo_TWO_MODEL
experiment_name=qwen2.5-1.5b-instruct-math500-twomodel-grpo-peerRwd-$backend
default_local_dir=$DATA_ROOT/checkpoint/$project_name/$experiment_name

# ============================================================================
# Two-model / peer-reward knobs
# ============================================================================
: "${THINKER_MODEL_PATH:=/root/autodl-tmp/models/Qwen2.5-1.5B-Instruct}"
: "${SOLVER_MODEL_PATH:=/root/autodl-tmp/models/Qwen2.5-1.5B-Instruct}"
: "${ALTERNATE_PERIOD:=10}"        # 1..N -> thinker, N+1..2N -> solver, ...
: "${START_WITH:=thinker}"
: "${TWO_MODEL_RUNTIME:=auto}"     # auto | checkpoint_swap | dual_worker
: "${PEER_REWARD_ETA:=0.3}"        # self-rating weight; peer-rating weight is 1-eta
: "${PEER_REWARD_LAMBDA:=0.2}"     # correct-answer credit shaping strength
: "${PEER_BLAME_LAMBDA:=0.2}"      # wrong-answer blame shaping strength
: "${PEER_CENTER_BY_GROUP:=True}"
: "${PEER_RATING_MAX_RESPONSE:=512}"
...
peer_reward_eta=$PEER_REWARD_ETA
peer_reward_lambda=$PEER_REWARD_LAMBDA
peer_blame_lambda=$PEER_BLAME_LAMBDA
peer_center_by_group=$PEER_CENTER_BY_GROUP
peer_rating_max_response=$PEER_RATING_MAX_RESPONSE
norm_adv_by_std_in_grpo=$NORM_ADV_BY_STD_IN_GRPO
swap_dir=$DATA_ROOT/checkpoint/$project_name/$experiment_name/_swap

# Per-role max response length. Thinker only emits the thought; Solver emits
# the final answer.
...
# Data / Model
# ============================================================================
train_files=/root/autodl-tmp/ccpo-verl-mulitAgent-peerRwd/data/MATH/train.parquet
VAL_VIEW_DIR=$DATA_ROOT/data/val_metric_views/qwen2_5_1_5b_grpo_twomodel_peerRwd
...
val_sets = [
    ("math500", "/root/autodl-tmp/ccpo-verl-mulitAgent-peerRwd/data/MATH/test500.parquet"),
    ("aime2024", "/root/autodl-tmp/ccpo-verl-mulitAgent-peerRwd/data/aime2024_fixed_with_prompt.parquet"),
    ("aime2025", "/root/autodl-tmp/ccpo-verl-mulitAgent-peerRwd/data/aime2025_fixed_with_prompt.parquet"),
    ("amc23", "/root/autodl-tmp/ccpo-verl-mulitAgent-peerRwd/data/amc23_fixed_with_prompt.parquet"),
    ("gaokao2023", "/root/autodl-tmp/ccpo-verl-mulitAgent-peerRwd/data/Gaokao2023-Math-En_with_prompt.parquet"),
    ("minervamath", "/root/autodl-tmp/ccpo-verl-mulitAgent-peerRwd/data/MinervaMath_with_prompt.parquet"),
]
...
# Initial actor path. The trainer swaps between thinker / solver during
# training.
actor_model_path=$thinker_model_path

# Max prompt length must accommodate problem + thinker thought for Solver.
...
# Actor config. model.path is the initial model; the trainer hot-swaps roles.
ACTOR_CONFIG="
...
TWO_MODEL_CONFIG="
    ...
    +two_model.peer_reward_eta=$peer_reward_eta \
    +two_model.peer_reward_lambda=$peer_reward_lambda \
    +two_model.peer_blame_lambda=$peer_blame_lambda \
    +two_model.peer_reward_center_by_group=$peer_center_by_group \
    +two_model.peer_rating_max_response=$peer_rating_max_response"
