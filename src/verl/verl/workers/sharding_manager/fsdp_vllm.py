...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __init__(
        ...
        # Handle different vLLM versions and executor types
        if inference_engine:
            model_executor = inference_engine.llm_engine.model_executor
            # For SPMDGPUExecutor (newer vLLM versions), access worker directly
            if hasattr(model_executor, 'worker'):
                self.model_runner = model_executor.worker.model_runner
            # For older vLLM versions with driver_worker
            elif hasattr(model_executor, 'driver_worker'):
                self.model_runner = model_executor.driver_worker.worker.model_runner
            else:
                raise AttributeError(f"Unknown model_executor type: {type(model_executor)}")
        else:
