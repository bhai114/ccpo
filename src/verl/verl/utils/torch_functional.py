...
def logprobs_from_logits(logits, labels):
    ...
    if FLAH_ATTN_CROSS_ENTROPY_LOSS_AVAILABLE:
        ...
        raise NotImplementedError('logprobs_from_logits_v2 should be modified to take IGNORE_INDEX into account')
...
def get_turn_mask(turn_level_reward_tensor: torch.Tensor, num_turns: List[int], dtype=torch.int64)->torch.Tensor:
    turn_mask = torch.ones_like(turn_level_reward_tensor, dtype=dtype)
    for i in range(turn_level_reward_tensor.shape[0]):
        turn_mask[i, num_turns[i]:] = 0
    return turn_mask
