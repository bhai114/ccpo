...
def _build_vllm_server_name(replica_rank: int, node_rank: int, name_prefix: str | None = None, is_reward_model: bool = False):
    parts = ["vllm_server_reward" if is_reward_model else "vllm_server"]
    if name_prefix:
        parts.append(str(name_prefix))
    parts.extend([str(replica_rank), str(node_rank)])
    return "_".join(parts)
...
class vLLMReplica(RolloutReplica):
    ...
    async def launch_servers(self):
        ...
        server_name_prefix = getattr(self.config.agent, "server_name_prefix", None)
