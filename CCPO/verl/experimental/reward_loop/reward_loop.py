...
class RewardLoopWorker:
    ...
    def set_input_tokenizer_path(self, model_path: str):
        """Reload the tokenizer used to decode rollout responses before scoring."""
        with open_dict(self.config):
            self.config.actor_rollout_ref.model.path = model_path
        input_tokenizer_local_path = copy_to_local(model_path)
        self.input_tokenizer = hf_tokenizer(input_tokenizer_local_path, trust_remote_code=True)
        self.reward_manager = load_reward_manager(
            self.config,
            self.input_tokenizer,
